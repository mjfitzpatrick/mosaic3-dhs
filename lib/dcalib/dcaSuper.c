...
fprintf (stderr, "dcaSupOpenSocket: '%s' %d -> '%s'\n", host, port,
...
/*      if (*ip == '\0' || strcmp (lhost, host_str) == 0) { */
        if (strcmp (lhost, host_str) == 0) { 
            strcpy (host_str, "localhost");
            if ((hp = gethostbyname(host_str))) {
fprintf (stderr, "localhost gethostbyname(host_str) = '%s'\n", host_str);
                bcopy (hp->h_addr, (char *)host_addr, sizeof(*host_addr));
	    }

        } else if (isdigit (host_str[0])) {
fprintf (stderr, "isdigit(host_str) = '%s'\n", host_str);
...
fprintf (stderr, "gethostbyname(host_str) = '%s'\n", host_str);
            bcopy (hp->h_addr, (char *)host_addr, sizeof(*host_addr));
