...
static int dhsPixelDataTcl ( ClientData clientData, Tcl_Interp *interp, int argc, char *argv[] ) {
 ...
 ival=0; if ( Tcl_GetInt(interp,lsArgvC[9],&ival)  != TCL_OK ) ierror = DHS_TRUE; fpConfigTcl.xAmpCnt  = (XLONG)ival;
 ival=0; if ( Tcl_GetInt(interp,lsArgvC[10],&ival) != TCL_OK ) ierror = DHS_TRUE; fpConfigTcl.yAmpCnt  = (XLONG)ival;
 if ( ierror ) {
