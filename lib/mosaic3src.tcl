...
proc DefHeader name {
    ...
    #if {[catch {FindValue EXPTIME}]} {MoveEntry $name "" "REQINTT EXPTIME"}
    if {[catch {FindValue EXPTIME}]} {
	set t [FindValue REQINTT]
	set t [expr $t - 2]
	AddEntry $name EXPTIME $t N "Requested - 2s"
    }
    #MoveEntry $name "" "NOCFIL   FILTER"   "Filter"
    MoveEntry $name "" "NOCTEL   TELESCOP"
    MoveEntry $name "" "NOCAO    OBSERVER"
    if {![catch {FindValue NOCOA}]} {MoveEntry $name "" "NOCOA OBSASST"}
...
proc PhuHeader name {
    ...
    if {![catch {set mjd [FindValue JDEXPS]}]} {
        set mjd [expr $mjd - 2400001.0 + (7. / 24.)]
	AddEntry phu MJDSTART $mjd N "MJD of observation start"
    }
    if {![catch {set mjd [FindValue JDEXPE]}]} {
        set mjd [expr $mjd - 2400001.0 + (7. / 24.)]
	AddEntry phu MJDEND $mjd N "MJD of observation end"
    }
...
proc ExtHeader {det} {
    ...
    #AddEntry $ext NAXIS1 [expr $detsize(amp,1)+$detsize(bias,1)+$detsize(extra,1)] N
    #AddEntry $ext NAXIS2 [expr $detsize(amp,2)+$detsize(bias,2)+$detsize(extra,2)] N
...
#    AddEntry  $ext      EXTVER   $extnum N "Extension version"
    ...
    } elseif {[info exists detval(gain,tbd)]} {
        AddEntry $ext GAIN $detval(gain,tbd) N "TBD Gain (e/ADU)"
    }
    if {[info exists detval(rdnoise,$det)]} {
        AddEntry $ext RDNOISE $detval(rdnoise,$det) N "Readout noise (e)"
    } elseif {[info exists detval(rdnoise,tbd)]} {
        AddEntry $ext RDNOISE $detval(rdnoise,tbd) N "TBD readout noise (e)"
    }
    if {[info exists detval(saturate,$det)]} {
        AddEntry $ext SATURATE $detval(saturate,$det) N "Saturation (ADU)"
    } elseif {[info exists detval(saturate,tbd)]} {
        AddEntry $ext SATURATE $detval(saturate,tbd) N "TBD saturation (ADU)"
    } elseif {[info exists detval(saturate,adc)]} {
        AddEntry $ext SATURATE $detval(saturate,adc) N "ADC saturation (ADU)"
    }

#    dprintf 0 "ExtHeader: Calling CcdGeom for det: " "$det\n"
#    dprintf 0 "ExtHeader: Calling CcdGeom for ccd: " "$ccd\n"
#    dprintf 0 "ExtHeader: Calling CcdGeom for amp: " "$amp\n"
    CcdGeom $ext $ccd $amp $ext
#    dprintf 0 "ExtHeader: Done Calling CcdGeom"  "0\n"
...
proc MosSave {} {
    ...
    DeleteEntry NOCS_Post UTDATE
    DeleteEntry NOCS_Post UT
    ...
    MoveEntry NOCS_Post NOCS_Pre UTDATE
    MoveEntry NOCS_Post NOCS_Pre UT
    CopyEntry NOCS_Post NOCS_Pre "MSEPEDF MSEPEDF1"
    CopyEntry NOCS_Post NOCS_Pre "FOCUS FOCUS1"
...
proc MapNOCS {inkwdb mapkwdb outroot mapunknown maplevel} {
	...
	for {set mep [kwdb_Head $mkwdb]} {$mep>0} {set mep [kwdb_Next $mkwdb $mep]} {
		...
		# Skip NOCS default values.
		if {[regexp {\[[se]\]} $value]} continue
	...
	# Skip NOCS default values.
	if {[regexp {\[[se]\]} $value]} continue
	# Skip NOHS duplication.
	if {$keyword == "NOHS" && $value != "Mosaic.1.1"} continue
...
proc CopyEntry {outkwdb inkwdb kws args} {
	...
	if {$value != ""} {
	    ...
	    dprintf 1 $imageid "CopyEntry: can't set value for keyword `$outkw'\n"
	    dprintf 2 $imageid "$err\n"
	}
    }
...
proc GetEntry {kwdbname kw} {
	...
	return -code error \
	    "GetEntry: keyword `$kw' from `$kwdbname' not found"
    }
    set kwdb $kwdbs($kwdbname)
    if {[catch {
...
proc FindEntry {kw} {
    ...
    return -code error "Find Entry: Error in GetEntry: keyword `$kw' not found"
}
...
proc SetObstype {outkwdb inkwdb kws args} {
	...
	dprintf 1 $imageid "SetObsType: can't set value for keyword `$outkw'\n"
	dprintf 2 $imageid "$err\n"
    }
}
...
proc SetEquin {outkwdb inkwdb rakw deckw eqkw} {
	...
	# Set EPOCH if not defined (which is the case for the 0.9m).
	if {[catch {FindValue EPOCH}]} {
	    CopyEntry $outkwdb "" "TELEQUIN EPOCH" "Epoch (yr)"
	}
...
proc SetTel {outkwdb inkwdb kws args} {
	...
	set tel [FindValue NOCSYS]
	switch $tel {
	"kpno 4m" {set telid kp4m}
	"kpno 36" {set telid kp09m}
	...
	kp4m {
		...
		"Mayall Corrector" S "Corrector Identification"
            ...
            CopyEntry $outkwdb $inkwdb  "MSEPEDF FOCEND"
            MoveEntry $outkwdb $inkwdb  "MSEPEDF1 FOCSTART"
            MoveEntry $outkwdb $inkwdb  "FOCUS1 TELFEND"
	    }
	kp09m {
	    kwdb_SetValue $kwdbs($outkwdb) $outkw "WIYN 0.9 meter telescope"
	    AddEntry  $outkwdb OBSERVAT WIYN S "Observatory"
	    AddEntry  $outkwdb TELID $telid S "Telescope ID"
	    AddEntry  $outkwdb RAPANGL 0. N "Position angle of RA axis (deg)"
	    ...
	    AddEntry  $outkwdb CORRCTOR \
		"WIYN 0.9m Corrector" S "Corrector Identification"
	    AddEntry  $outkwdb ADC "none" S "ADC Identification"

            CopyEntry $outkwdb $inkwdb  "FOCUS FOCEND"
            MoveEntry $outkwdb $inkwdb  "FOCUS1 FOCSTART"
            MoveEntry $outkwdb $inkwdb  "MSEPEDF1 PEDFEND"
	    }
	}
    } err]} {
	dprintf 1 $imageid \
...
proc SetFocus {outkwdb inkwdb} {
	...
	MoveEntry $outkwdb def "FOCEND TELFOCUS"

	# Do focus sequence.
	if {[GetValue $outkwdb OBSTYPE] == "focus"} {
	    MoveEntry $outkwdb def FOCSTART
...
proc SetFilter {outkwdb inkwdb} {
	...
	dprintf 1 $imageid "SetFilter: can't set value for keyword FILTER\n"
	dprintf 2 $imageid "$err\n"
    }
}
...
# DLR DEBUG
#     dprintf 0 "Starting CcdGeom:  ccd: " "$ccd\n"
#     dprintf 0 "Starting CcdGeom:  amp: " "$amp\n"
	...
	set ltmbin(1) $ltm(1); set ltmbin(2) $ltm(2)
	set ltvbin(1) $ltv($amp,1); set ltvbin(2) $ltv($amp,2)

	# Set the sections.
	set ccdsec [format "\[%d:%d,%d:%d\]" $cx1 $cx2 $cy1 $cy2]
#     dprintf 0 " CcdGeom:  ccdsec: " "$ccdsec\n"
...
#     dprintf 0 " CcdGeom:  datasec: " "$datasec\n"

	set bc1 [expr $detsize(amp,1)+$detsize(extra,1)+1]
	set bc2 [expr $bc1+$detsize(bias,1)-1]
	set bl1 $il1
	set bl2 $il2
	set biassec [format "\[%d:%d,%d:%d\]" $bc1 $bc2 $bl1 $bl2]
#     dprintf 0 " CcdGeom:  biassec: " "$biassec\n"
...
#     dprintf 0 " CcdGeom:  detsec: " "$detsec\n"
...
#     dprintf 0 " CcdGeom:  ampsec: " "$ampsec\n"
