...
main(int argc, char *argv[])
{
    ...
    */
    printf("Initializing connections to message bus\n");fflush(stdout);
    if ((mytid = mbusConnect("CMD", group, FALSE)) <= 0) {
	fprintf(stderr, "ERROR: Can't connect to message bus.\n");fflush(stderr);
	exit(1);
    }
    printf("Done initializing connections to message bus\n");fflush(stdout);
    ...
    printf("Done processing command line arguments\n");fflush(stdout);


    if (interactive) {

        printf("Entering interactive loop\n");fflush(stdout);
        printf ("\n\n\n");
        while (1) {
            printf ("Hit <cr> to begin exposure, 'q' to quit....");fflush(stdout);
            if (fgets (buf, 80, stdin) == NULL || buf[0] == '\n') {

		sprintf (msg, "%.4f", (expID += 0.1));
		printf ("Starting exposure #%d:  ID:%.2lf  ", 
		    ++expnum, expID);fflush(stdout);

                printf ("Sending exposure to NOCS\n");fflush(stdout);
	        sendExposure ("NOCS", expID);

                printf ("Done sending exposure to NOCS\n");fflush(stdout);
            } else if (buf[0] == 'q')
                break;
        ...
        printf("Done with interactive loop\n");fflush(stdout);

    } else {
        printf("Entering non-interactive loop\n");fflush(stdout);
        for (i=1; i <= loop; i++) {
	    fprintf (stderr, "%s Exp #%d  ID:%.2lf \n",
		pTime(buf), i, (expID += 0.1));fflush(stderr);
        ...
        printf("Done with non-interactive loop\n");fflush(stdout);
    }
